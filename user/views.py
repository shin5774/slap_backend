...
class MemberListAPI(viewsets.ModelViewSet):
    ...
    existQueryset = User.objects.filter(is_delete='0')
    ph=PasswordHasher()
    ...
    # user/{key}/change_password/?password={}&apaasword={}
    ...
    #[post] user/password_check 회원가입시 or 비밀번호 변경시 비밀번호 2번 입력하고 확인하는 과정을 따로 기능으로 만듬
    @action(detail=False,methods=['POST'])
